    ...
    public function uploadFolder(Request $request)
    {
        if ($_POST) {
            ...
            if ($file_error > 0) { // 出错
                ...
                $folder['father_id'] = session('folder_id');
                $folder['uid'] = session('userSession')->id;
                $folder['create_time'] = date('Y-m-d H:i:s');
                $folder['update_time'] = date('Y-m-d H:i:s');
                $folder['is_share'] = session('is_share');
                $res_folder = DB::table('netdisk_folders')->where(['is_share' => session('is_share'),'father_id' => session('folder_id'), 'folder_name' => $folder_name])->first();
                if (!empty($res_folder)) {
                    $folder_id=$res_folder->id;
                    DB::table('netdisk_folders')->where('id',$folder_id)->update(['is_delete'=>0]);
                }else{
                    $folder_id = DB::table('netdisk_folders')->insertGetId($folder);
                }
                $data['file_size'] = $file_size;
                $data['file_name'] = $file_name;
                $data['file_type'] = $file_name_arr[count($file_name_arr) - 1];
